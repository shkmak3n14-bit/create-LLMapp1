...
def create_rag_chain(db_name):
    ...
    # 既存のベクトルDBがある場合でも、元データの変更を確実に反映するため毎回再構築する
...
def build_retrieval_input(text):
    """
    検索時の表記ゆれを吸収するため、入力テキストを正規化して別表記を補う
    ...
    compact_english = re.sub(r"(?<=[A-Za-z])\s+(?=[A-Za-z])", "", normalized_text)

    if compact_english != normalized_text:
        return "\n".join([normalized_text, f"別表記: {compact_english}"])
...
def run_company_doc_chain(param):
    ...
    Returns:
        LLMからの回答
    """
    retrieval_input = build_retrieval_input(param)
    # 会社に関するデータ参照に特化したChainを実行してLLMからの回答取得
    ai_msg = st.session_state.company_doc_chain.invoke({"input": retrieval_input, "chat_history": st.session_state.chat_history})
    # 会話履歴への追加
...
def run_all_doc_chain(param):
    ...
    """
    retrieval_input = build_retrieval_input(param)
    ai_msg = st.session_state.rag_chain.invoke({"input": retrieval_input, "chat_history": st.session_state.chat_history})
    st.session_state.chat_history.extend([HumanMessage(content=param), AIMessage(content=ai_msg["answer"])])
...
def run_service_doc_chain(param):
    ...
    ai_msg = st.session_state.service_doc_chain.invoke({"input": retrieval_input, "chat_history": st.session_state.chat_history})
...
def run_customer_doc_chain(param):
    ...
    ai_msg = st.session_state.customer_doc_chain.invoke({"input": retrieval_input, "chat_history": st.session_state.chat_history})
...
def execute_agent_or_chain(chat_message):
    ...
    """
    logger = logging.getLogger(ct.LOGGER_NAME)
    retrieval_input = build_retrieval_input(chat_message)

    # AIエージェント機能を利用する場合
    if st.session_state.agent_mode == ct.AI_AGENT_MODE_ON:
        # まずは全体RAGで社内文書を優先して回答する
        rag_result = st.session_state.rag_chain.invoke({"input": retrieval_input, "chat_history": st.session_state.chat_history})
        rag_response = rag_result["answer"]
        if rag_response != ct.NO_DOC_MATCH_MESSAGE:
            st.session_state.chat_history.extend([HumanMessage(content=chat_message), AIMessage(content=rag_response)])
            if rag_response != ct.NO_DOC_MATCH_MESSAGE:
        ...
        result = st.session_state.agent_executor.invoke({"input": retrieval_input}, {"callbacks": [st_callback]})
        response = result["output"]
    # AIエージェントを利用しない場合
    else:
        # RAGのChainを実行
        result = st.session_state.rag_chain.invoke({"input": retrieval_input, "chat_history": st.session_state.chat_history})
        # 会話履歴への追加
